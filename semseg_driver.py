...
import pygame
import numpy as np
from buw_functions import get_ego_id

from buw_functions import model_settings, inference, carla_colorize
...
class CameraManager(object):
    def __init__(self, parent_actor, res, model_name='fast_scnn'):
        self.sensor = None
        self.surface = None
        ...
        self.index = None
        ############################ add inference #############################
        self.network_1, self.network_2, self.torch_transform, self.id2color = model_settings(self.model_name)
        ########################################################################
    ...
    def _parse_image(weak_self, image):
        ...
        if not self:
            return
        mask = inference(image, self.network_1, self.torch_transform, self.model_name)
